...
b1_dataloader_path = "data/part1/train_glove_dataloader_1perc"
b2_dataloader_path = "data/part2/dataloader"
aucs = []

encoder = ConvNet(300, hidden_dim, 3, pad=1)
encoder.double()
...
if is_cuda:
    ...
    lmbda = Variable(torch.cuda.DoubleTensor([.001]))
    eps = Variable(torch.cuda.DoubleTensor([.001]))
    two = Variable(torch.cuda.DoubleTensor([2]))
else:
    lmbda = Variable(torch.DoubleTensor([.001]))
    eps = Variable(torch.DoubleTensor([.001]))
    two = Variable(torch.DoubleTensor([2]))

labels_optimizer = optim.Adam(encoder.parameters(), lr = 0.005)
domain_optimizer = optim.Adam(domain_discriminator.parameters(), lr=-.005)
...
for epoch in xrange(num_epochs):
    ...
    for b1, b2 in tqdm(itertools.izip(b1_dataloader, b2_dataloader)):
        ...
        b1_title, b1_body = Variable(b1_title), Variable(b1_body)
        b1_title_mask, b1_body_mask = Variable(b1_title_mask), Variable(b1_body_mask)
        b2_title, b2_body, b2_labels = Variable(b2_title), Variable(b2_body), Variable(b2_labels)
        b2_title_mask, b2_body_mask = Variable(b2_title_mask), Variable(b2_body_mask)
        ...
        # apply mask
        b1_titles_encoded = b1_title * b1_title_mask.unsqueeze(1).expand_as(b1_title)
        b1_bodies_encoded = b1_body * b1_body_mask.unsqueeze(1).expand_as(b1_body)

        b2_titles_encoded = b2_title * b2_title_mask.unsqueeze(1).expand_as(b2_title)
        b2_bodies_encoded = b2_body * b2_body_mask.unsqueeze(1).expand_as(b2_body)

        # average over words 
        b1_titles_encoded = torch.sum(b1_titles_encoded, dim=2) 
        b1_bodies_encoded = torch.sum(b1_bodies_encoded, dim=2)

        b2_titles_encoded = torch.sum(b2_titles_encoded, dim=2) 
        b2_bodies_encoded = torch.sum(b2_bodies_encoded, dim=2)

        # (div by actual length)
        b1_titles_encoded = b1_titles_encoded / (torch.sum(b1_title_mask, keepdim=True, dim=1).expand_as(b1_titles_encoded) + eps)
        b1_bodies_encoded = b1_bodies_encoded / (torch.sum(b1_body_mask, keepdim=True, dim=1).expand_as(b1_bodies_encoded) + eps)

        b2_titles_encoded = b2_titles_encoded / (torch.sum(b2_title_mask, keepdim=True, dim=1).expand_as(b2_titles_encoded) + eps)
        b2_bodies_encoded = b2_bodies_encoded / (torch.sum(b2_body_mask, keepdim=True, dim=1).expand_as(b2_bodies_encoded) + eps)
    ...
    prev_encoder = ConvNet(300, hidden_dim, 3, pad=1, dropout=0)
    prev_encoder.double()
